...
三.
	1.当str是普通值的时候,对应v的类型是main.field, 调用go v.print的时候,调用的是v这个临时变量的地址去调用方法, v这个局部变量的地址是不变的,所以p实际是确定的,变的只是它存的值,
	...
	3.如果print方法是func (f field)print的时候,这里str是普通值或者指针都可以
